...
import traceback
import sys
from app.constant import CvatUrls
from cvat_utils import login
...
class RetrieveTaskDetails:
    def __init__(self):
        try:
            self.CVAT_USER = os.getenv('CVAT_USER', 'user')
            self.CVAT_MAIL = os.getenv('CVAT_MAIL', '')
            self.CVAT_PASS = os.getenv('CVAT_PASS', 'user')
            self.CVAT_DOMAIN = os.getenv('CVAT_DOMAIN', '192.168.12.85:8080')
            self.CVAT_PROTO = os.getenv('CVAT_PROTO', 'http')
            self.token = login()
            if None in (
