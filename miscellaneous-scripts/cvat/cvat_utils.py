import requests
from constant import CVAT_PROTO, CVAT_DOMAIN, CVAT_MAIL, CVAT_PASS, CVAT_USER, CVAT_IMPORT_FORMAT
import urllib
...
async def get_projects_dict(token):
...
async def create_task(token, task_name, project_name):
    try:
        url = f'{CVAT_PROTO}://{CVAT_DOMAIN}/api/v1/tasks'
        projects_dict = await get_projects_dict(token)
        payload = {"name": task_name, "labels": [], "project_id": projects_dict[project_name]}
