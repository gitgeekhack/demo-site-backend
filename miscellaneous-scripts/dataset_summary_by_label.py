...
list_of_labels = ['FILENAME', 'SUBTITLE', 'CONTENT', 'FORM', 'TABLE', 'REP_HEADER_FOOTER', 'HEADER_FOOTER', 'TITLE',
                  'QUESTIONNAIRE', 'OTHER']
DATASET_PATH = "/home/heli/Documents/git/pareIT-internal/download_dataset/"  # path to your dataset
SUMMARY_CSV_PATH = f'/home/heli/Documents/git/pareIT-internal/DatasetSummary.csv'  # path for the summary csv
files = glob.glob(os.path.join(DATASET_PATH, '*'))  # gets all file names from dataset
ANNOTATION_FILE = 'annotations.xml'
MODULE_NAME = 'Document_Structure'
DELIMINATOR = '</meta>'


async def csv_writer(summary):
    with open(SUMMARY_CSV_PATH, 'a+') as csvfile:
