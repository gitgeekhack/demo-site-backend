...
def insert_annotation_type(conn, annotation_type):
    """
...
def insert_label_details(conn, annotation_type_id, page_no, label_id):
    """
...
def insert_sublabel_detail(conn, label_detail_id, sublabels):
    """
...
def insert_annotation_detail(conn, label_detail_id, annotations):
    """
...
def insert_task_label_map(conn, label_detail_id, task_name):
    """
...
def insert_annotations(conn, task_details):
    """
    ...
    if task_details['Labels']:
        for label in task_details['Labels']:
            label_id = insert_label(conn, label['LabelName'])
            annotation_type_id = insert_annotation_type(conn, label['AnnotationType'])
            label_detail_id = insert_label_details(conn, annotation_type_id, label['PageNo'], label_id)
            if label['SubLabels']:
                insert_sublabel_detail(conn, label_detail_id, label['SubLabels'])
            insert_annotation_detail(conn, label_detail_id, label['Annotations'])
            insert_task_label_map(conn, label_detail_id, task_details['TaskName'])



def add_annotation_file_s3_url(conn, url, task_id):
    """
...
def update_task_details(conn, task_details, url):
    """
    ...
    if data:
        ...
        if url:
            add_annotation_file_s3_url(conn, url, task_id)
