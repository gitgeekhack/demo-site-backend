...
FILE_LIST_PATH = "/home/ubuntu/git/pareit-miscellaneous-scripts/stats/all_s3_objects_08122022.txt"  # file contains list of filenames to be downloaded from s3
SUCCESSFULLY_DOWNLOADED_LIST_PATH = "./successfully_downloaded_08122022_1920.txt"  # file contains list of filenames that are successfully downloaded
UNSUCCESSFULLY_DOWNLOADED_LIST_PATH = "./unsuccessfulfiles_08122022_1920.txt"  # file contains list of filenames that couldn't complete the download process
SSE_KEY_PATH = "/home/ubuntu/git/pareit-miscellaneous-scripts/stats/s3_sse_key.bin"  # file contains SSE C Decryption key
SAVE_FOLDER_NAME = '/mnt/dataset_volume/hashed_dataset_08122022'
...
successful_q = Queue()
unsuccessful = Queue()
FILTER_SUBSTRING_LIST = ['Handwritten_Checkbox', 'Document Structure', 'Split_Turn', 'images.gz']
pbar = tqdm(total=54388)
...
async def check_task_name(name):
    return any(word in name for word in FILTER_SUBSTRING_LIST) or name.lower().endswith('.pdf')
...
async def download_from_s3():
    while not files_q.empty():
        ...
        try:
            ...
            if file_path not in successful_q.queue:
...
async def update_unsuccessful_queue(object_name):
    ...
    async with aiofiles.open(UNSUCCESSFULLY_DOWNLOADED_LIST_PATH, "a") as unsuccessful_obj:
        await unsuccessful_obj.writelines([object_name])
...
async def update_success_queue(object_name):
    ...
    async with aiofiles.open(SUCCESSFULLY_DOWNLOADED_LIST_PATH, "a") as successful_obj:
        await successful_obj.writelines([object_name])
...
async def main():
    await load_queues()
    download_coroutines = [download_from_s3() for _ in range(64)]
