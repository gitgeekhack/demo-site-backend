...
# load dataset
dataset = read_csv('outlier_removed.csv')
...
makes = set(values[:, 4])
categories = set(values[:, 5])
import numpy as np

for make in makes:
    for category in categories:
        x = values[(values[:, 4] == make) & (values[:, 5] == category)]
...
# split_index = len(dataset) - int(len(dataset) * 0.10)
...
model.add(Dense(1))
model.compile(loss=var_loss_eval, optimizer=var_optimizer)
# fit network
history = model.fit(train_X, train_y, epochs=epoch_var, batch_size=batch, validation_data=(test_X, test_y), verbose=1,
                    shuffle=False)
model.save('model')
# plot history
