from app.constant import SRC_INSURANCE_APPLICATION, TRG_ITC, TRG_PUL, SRC_VR, SRC_REGISTRATION, TRG_AUL, TRG_INS_APP
from app.service.api.v1.insurance_application.verifier_v1.alliance_united import vehicle_information
...
valid_data = {"promise_to_provide": {"signature": {"agreed_to_be": {"is_signed": True},
                           ...
                           "promise_to_provide_agreement_date": ["2021-10-01", "2021-10-01", "2021-10-01"]}, 'mvr': [
    ...
    'pleasure_use_letters': {'insured_name': 'OLIVARES WENDY BANESA', 'policy_number': 'MIL4972376',
                              'vehicles': [
                                  {'year': 2010, 'make': 'TOYT', 'model': 'VENZA', 'vin': '4T3ZA3BB1AU023479'}],
...
application = {'id': 1, 'year': 2005, 'make': 'FORD', 'model': 'RANGER', 'trim': None, 'vehicle_use': 'Pleasure',
               'vin': '1FTYR10D45PA79138'}
itc_dict = {'id': 1, 'vin': '1FTYR10D45PA79138', 'make': 'FORD', 'model': 'RANGER', 'year': 2005,
            'annual_miles_driven': 3500, 'comprehensive_deductible': None, 'collision_deductible': None}

application_vehicle = {'id': 1, 'year': 2005, 'make': 'FORD', 'model': 'RANGER', 'trim': None,
                       'vehicle_use': 'Pleasure', 'vin': '1FTYR10D45PA79138'}
pleasure_use_letters = [{'year': 2005, 'make': 'FORD', 'model': 'RANGER', 'vin': '1FTYR10D45PA79138'},
                        {'year': 2005, 'make': 'MAZD', 'model': 'MAZDA6I', 'vin': '1YVHP80C355M69899'}]
artisan_use_letter = [{'year': 2005, 'make': 'FORD', 'model': 'RANGER', 'vin': '1FTYR10D45PA79138'},
                      {'year': 2005, 'make': 'MAZD', 'model': 'MAZDA6I', 'vin': '1YVHP80C355M69899'}]
vr = [{'vehicle': {'year': 2005, 'make': 'FORD', 'model': 'RANGER', 'vin': '1FTYR10D45PA79138'}}]
vr_false = [{'vehicle': {'year': 2003, 'make': 'FORA', 'model': 'RANGOR', 'vin': '1FTYR10D45PA79138'}}]
registration = [
    {'vehicle': {'year': 2005, 'make': 'MAZD', 'model': 'MAZDA6I', 'vin': '1YVHP80C355M69899'}, 'is_valid': True}]
registration_false = [
    {'vehicle': {'year': 2003, 'make': 'MAAD', 'model': 'MAZDA7I', 'vin': '1YVHP80C355M69899'}, 'is_valid': False}]


async def test__verify_vehicle_info_valid1():
    data = await deserialize(valid_data)
    data.insurance_application.vehicles = await deserialize(application_vehicle)
    data.pleasure_use_letter.vehicles = await deserialize(pleasure_use_letters)
    data.artisan_use_letter.vehicles = await deserialize(artisan_use_letter)
    data.vr = await deserialize(vr)
    data.registration = await deserialize(registration)
    obj = vehicle_information.VehicleInformationVerifier(uuid=uuid, data=data)
    x = await obj._VehicleInformationVerifier__verify_vehicle_info()
    response = [{'id': 1,
                 'make': {'source': SRC_VR,
                          'target': {TRG_ITC: True,
                                     TRG_PUL: True,
                 ...
                 'vin': {'source': SRC_VR,
                         'target': {TRG_ITC: True,
                                    ...
                                    TRG_AUL: True,
                                    TRG_INS_APP: True}},
                 'year': {'source': SRC_VR, 'target': {TRG_ITC: True,
                                                     TRG_PUL: True,
                                                     TRG_AUL: True,
                                                     TRG_INS_APP: True}},
                 'is_valid': None},
                {'id': 2,
                 'make': {'source': 'registration',
                          'target': {TRG_ITC: True, TRG_PUL: True,
                                     TRG_AUL: True,
                                     TRG_INS_APP: True}},
                 'model': {'source': 'registration',
                           'target': {TRG_ITC: True,
                                      ...
                                      TRG_AUL: True,
                                      TRG_INS_APP: True}},
                 'vin': {'source': 'registration',
                         'target': {TRG_ITC: True,
                                    TRG_PUL: True,
                                    TRG_AUL: True,
                                    TRG_INS_APP: True}},
                 'year': {'source': 'registration',
                          'target': {TRG_ITC: True,
                                     TRG_PUL: True,
                                     TRG_AUL: True,
                                     TRG_INS_APP: True}},
                 'is_valid': {'source': 'registration',
                              'target': {'registration': True}}}]

    assert x == response


async def test__verify_vehicle_info_valid2():
    data = await deserialize(valid_data)
    data.insurance_application.vehicles = await deserialize(application_vehicle)
    data.pleasure_use_letter.vehicles = await deserialize(pleasure_use_letters)
    data.artisan_use_letter.vehicles = await deserialize(artisan_use_letter)
...
async def test__verify_vehicle_info_valid3():
    data = await deserialize(valid_data)
    data.insurance_application.vehicles = await deserialize(application_vehicle)
    data.pleasure_use_letter.vehicles = await deserialize(pleasure_use_letters)
    data.artisan_use_letter.vehicles = await deserialize(artisan_use_letter)
...
async def test__verify_vehicle_info_valid4():
    data = await deserialize(valid_data)
    data.insurance_application.vehicles = await deserialize(application_vehicle)
    data.pleasure_use_letter.vehicles = await deserialize(pleasure_use_letters)
    data.artisan_use_letter.vehicles = await deserialize(artisan_use_letter)
...
async def test__verify_vehicle_info_valid5():
    data = await deserialize(valid_data)
    data.insurance_application.vehicles = await deserialize(application_vehicle)
    data.pleasure_use_letter.vehicles = await deserialize(pleasure_use_letters)
    data.artisan_use_letter.vehicles = await deserialize(artisan_use_letter)
    data.vr = await deserialize(vr_false)
    data.registration = await deserialize(registration_false)
    obj = vehicle_information.VehicleInformationVerifier(uuid=uuid, data=data)
    x = await obj._VehicleInformationVerifier__verify_vehicle_info()
    response = [{'id': 1,
                 'make': {'source': SRC_VR,
                          'target': {TRG_ITC: False,
                                     TRG_PUL: False,
                                     TRG_AUL: False,
                                     TRG_INS_APP: False}},
                 'model': {'source': SRC_VR,
                 ...
                 'vin': {'source': SRC_VR,
                         'target': {TRG_ITC: True,
                                    ...
                                    TRG_AUL: True,
                                    TRG_INS_APP: True}},
                 'year': {'source': SRC_VR, 'target': {TRG_ITC: False,
                                                     TRG_PUL: False,
                                                     TRG_AUL: False,
                                                     TRG_INS_APP: False}},
                 'is_valid': None},
                {'id': 2,
                 'make': {'source': 'registration',
                          'target': {TRG_ITC: False, TRG_PUL: False,
                                     TRG_AUL: False,
                                     TRG_INS_APP: False}},
                 'model': {'source': 'registration',
                           'target': {TRG_ITC: False,
                 ...
                 'year': {'source': 'registration',
                          'target': {TRG_ITC: False,
                                     ...
                                     TRG_INS_APP: False}},
                 'is_valid': {'source': 'registration',
                              'target': {'registration': False}}}]

    assert x == response


async def test__verify_vehicle_info_valid6():
    data = await deserialize(valid_data)
    data.insurance_application.vehicles = await deserialize(application_vehicle)
    ...
    response = [{'id': 1,
                ...
                {'id': 2,
                 'make': {'source': 'registration',
                          'target': {TRG_ITC: True, TRG_PUL: None,
                                     TRG_AUL: True,
                                     TRG_INS_APP: True}},
                 'model': {'source': 'registration',
                           'target': {TRG_ITC: True,
...
async def test__verify_vehicle_info_valid7():
    data = await deserialize(valid_data)
    data.insurance_application.vehicles = await deserialize(application_vehicle)
    data.pleasure_use_letter.vehicles = await deserialize(pleasure_use_letters)
    data.artisan_use_letter = None
    data.vr = await deserialize(vr)
    data.registration = await deserialize(registration)
    obj = vehicle_information.VehicleInformationVerifier(uuid=uuid, data=data)
    x = await obj._VehicleInformationVerifier__verify_vehicle_info()
    response = [{'id': 1,
                 ...
                 'is_valid': None},
                {'id': 2,
                 'make': {'source': SRC_REGISTRATION,
                          'target': {TRG_ITC: True, TRG_PUL: True,
                                     TRG_AUL: None,
                                     TRG_INS_APP: True}},
                 'model': {'source': SRC_REGISTRATION,
                           'target': {TRG_ITC: True,
                                      TRG_PUL: True,
                                      TRG_AUL: None,
                                      TRG_INS_APP: True}},
                 'vin': {'source': SRC_REGISTRATION,
                         'target': {TRG_ITC: True,
                                    TRG_PUL: True,
                                    TRG_AUL: None,
                                    TRG_INS_APP: True}},
                 'year': {'source': SRC_REGISTRATION,
                          'target': {TRG_ITC: True,
                                     TRG_PUL: True,
                                     TRG_AUL: None,
                                     TRG_INS_APP: True}},
                 'is_valid': {'source': SRC_REGISTRATION,
                              'target': {SRC_REGISTRATION: True}}}]
