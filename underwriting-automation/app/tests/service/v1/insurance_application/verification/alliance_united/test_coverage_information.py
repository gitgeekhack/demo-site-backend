...
from app.service.api.v1.insurance_application.verifier_v1.alliance_united import coverage_information
...
async def test_get_coverage_info_for_vehicles_valid():
    data = await deserialize(valid_data)
    obj = coverage_information.CoverageInformationVerifier(uuid=uuid, data=data)
    x = await obj._CoverageInformationVerifier__get_coverage_info_for_vehicles()
    response = [{'id': 1, 'comprehensive_deductible': {'source': SRC_ITC, 'target': {TRG_INS_APP: True}},
                 'collision_deductible': {'source': SRC_ITC, 'target': {TRG_INS_APP: True}}}]
    assert x == response


async def test__verify_coverage_info_valid():
    data = await deserialize(valid_data)
    obj = coverage_information.CoverageInformationVerifier(uuid=uuid, data=data)
    x = await obj._CoverageInformationVerifier__verify_coverage_info()
    response = {'bodily_injury': {'source': SRC_ITC, 'target': {TRG_INS_APP: True}},
                'property_damage': {'source': SRC_ITC, 'target': {TRG_INS_APP: True}},
                'uninsured_bodily_injury': None,
                'uninsured_property_damage': None, 'vehicles': [
            {'id': 1, 'comprehensive_deductible': {'source': SRC_ITC, 'target': {TRG_INS_APP: True}},
             'collision_deductible': {'source': 'itc', 'target': {TRG_INS_APP: True}}}]}
    assert x == response


async def test_get_coverage_info_for_vehicles_invalid():
    data = await deserialize(valid_data)
    obj = coverage_information.CoverageInformationVerifier(uuid=uuid, data=data)
    test = valid_data
    test['itc']['vehicle_information'] = None
    obj.itc = await deserialize(test)
    x = await obj._CoverageInformationVerifier__get_coverage_info_for_vehicles()
    assert not x
