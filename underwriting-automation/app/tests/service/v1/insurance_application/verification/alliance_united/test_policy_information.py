from app.constant import TRG_CRM_RECEIPT, SRC_INSURANCE_APPLICATION, TRG_EFT, TRG_PUL, \
    TRG_AUL, TRG_NOL
from app.service.api.v1.insurance_application.verifier_v1.alliance_united import policy_information
...
async def test__verify_policy_number():
    data = await deserialize(valid_data)
    obj = policy_information.PolicyInformationVerifier(uuid=uuid, data=data)
    response = {'source': SRC_INSURANCE_APPLICATION,
                'target': {TRG_CRM_RECEIPT: True, TRG_EFT: True, TRG_PUL: True,
                           TRG_AUL: True, TRG_NOL: True}}
    x = await obj._PolicyInformationVerifier__verify_policy_number()
    assert x == response


async def test__verify_policy_number_false():
    data = await deserialize(valid_data)
    obj = policy_information.PolicyInformationVerifier(uuid=uuid, data=data)
    obj.crm_receipt.policy_number = 'ABC12345'
    obj.pleasure_use_letter.policy_number = 'ABC12345'
    obj.artisan_use_letter.policy_number = 'ABC12345'
    obj.eft.policy_number = 'ABC12345'
    obj.non_owners_letter.policy_number = 'ABC12345'
    response = {'source': SRC_INSURANCE_APPLICATION,
                'target': {TRG_CRM_RECEIPT: False, TRG_EFT: False, TRG_PUL: False,
                           TRG_AUL: False, TRG_NOL: False}}
    x = await obj._PolicyInformationVerifier__verify_policy_number()
    assert x == response


async def test__verify_policy_number_none():
    data = await deserialize(valid_data)
    obj = policy_information.PolicyInformationVerifier(uuid=uuid, data=data)
    obj.crm_receipt.policy_number = None
    obj.pleasure_use_letter.policy_number = None
    obj.artisan_use_letter.policy_number = None
    obj.eft.policy_number = None
    obj.non_owners_letter.policy_number = None
    response = {'source': SRC_INSURANCE_APPLICATION,
                'target': {TRG_CRM_RECEIPT: None, TRG_EFT: None, TRG_PUL: None,
                           TRG_AUL: None, TRG_NOL: None}}
    x = await obj._PolicyInformationVerifier__verify_policy_number()
    assert x == response


async def test__verify_policy_number_source_none():
    data = await deserialize(valid_data)
    obj = policy_information.PolicyInformationVerifier(uuid=uuid, data=data)
    obj.application.policy_information.policy_number = None
    x = await obj._PolicyInformationVerifier__verify_policy_number()
    assert not x
