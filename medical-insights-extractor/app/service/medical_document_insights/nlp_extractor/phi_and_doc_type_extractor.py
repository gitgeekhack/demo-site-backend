...
class PHIAndDocTypeExtractor:
    ...
    async def __get_docs_embeddings(self, data):
        ...
        logger.info(f'[PHI-Embeddings] Chunk Preparation Time: {emb_start_time - chunk_start_time}')

        vector_embeddings = FAISS.from_documents(
            documents=docs,
            embedding=self.bedrock_embeddings,
        )
        logger.info(f'[PHI-Embeddings][{self.model_embeddings}]'
    ...
    async def get_patient_information(self, data):
        ...
        logger.info(f"[PHI] PHI Dates Extraction is completed in {time.time() - dates_start_time} seconds.")
