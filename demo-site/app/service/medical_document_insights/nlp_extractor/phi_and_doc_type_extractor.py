...
class PHIAndDocTypeExtractor:
    ...
    async def get_patient_information(self, data):
        ...
        t = time.time()
        logger.info("[Medical-Insights] Identifying of Document Type is started...")
        document_type = await self.__get_document_type(embeddings)
        logger.info(f"[Medical-Insights] Identification of Document Type is completed in {time.time() - t} seconds.")
        ...
        logger.info(f"[Medical-Insights] PHI Dates Extraction is completed in {time.time() - t} seconds.")

        t = time.time()
        logger.info("[Medical-Insights] Patient Name and DOB Extraction is started...")
        patient_name_and_dob = await self.__get_patient_name_and_dob(embeddings)
        logger.info(f"[Medical-Insights] Patient Name and DOB Extraction is completed in {time.time() - t} seconds.")
